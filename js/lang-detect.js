/* ============================================================
   Rimac Nevera 自动语言检测 — 每次加载页面都检查访问者 IP
   - 海外 IP(非 CN)访问中文页  → 跳转英文版 en.html
   - 国内 IP 访问英文版         → 跳转中文版 index.html
   - 用户主动点过语言按钮       → 尊重用户选择,不再自动跳转
   - IP 检测失败(超时/被墙)   → 保持当前语言,不打扰
   注:click 监听必须最先注册(先于任何提前 return),
   否则已选当前语言的用户点「切换」时不会记录新选择,会被 IP 检测弹回。
   ============================================================ */
(function () {
  document.addEventListener('click', function (ev) {
    ...
    try { localStorage.setItem('nevera-lang', lang); } catch (e) {}
  });
  ...
  var saved = null;
  try { saved = localStorage.getItem('nevera-lang'); } catch (e) {}

  if (saved === (onEn ? 'en' : 'zh')) return;            // 用户显式选择过当前语言
